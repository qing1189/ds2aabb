...
async function getWasm() {
  ...
  const wasmPath = join(__dirname, '..', 'sha3_wasm_bg.wasm');
...
function deepSeekHashV1(data) {
  ...
  buf.copy(tail, 0, off);
  tail[buf.length - off] = 0x06;
  tail[RATE - 1] |= 0x80;
  for (let i = 0; i < RATE / 8; i++) s[i] = (s[i] ^ readU64LE(tail, i * 8)) & MASK;
  keccakF23(s);
  const out = Buffer.alloc(32);
  for (let i = 0; i < 4; i++) writeU64LE(s[i], out, i * 8);
  return out;
}
...
let wasmReady = getWasm().catch(() => null);

parentPort.on('message', async ({ id, challenge, salt, expireAt, difficulty, useWasm }) => {
  try {
    const prefix = `${salt}_${expireAt}_`;
    let answer;
    ...
    if (useWasm) {
      ...
      if (instance) {
        answer = wasmSolve(instance, challenge, prefix, difficulty);
      } else {
        answer = solvePowBigInt(challenge, salt, expireAt, difficulty);
      }
    } else {
      answer = solvePowBigInt(challenge, salt, expireAt, difficulty);
    }

    parentPort.postMessage({ id, answer });
  } catch (err) {
